...
Hãy cho tôi biết vị trí của những hàm dưới đây sẽ ở trong những module cụ thể nào nếu áp dụng autosar
	Mô tả luồng dữ liệu trong project của
	tôi(chưa áp dụng autosar layer triệt để)
	/* Bsw --> module Servo : Servo_Init(), Led_Init(), LedTimerInterruptEnable() ,  LedTimerInterruptDisable()
	   App --> module Control_output
				--> SignalOutput_ModuleInit() --call--> Servo_Init() , Led_Init()
				--> Led_Control() --call--> LedTimerInterruptEnable() ,  LedTimerInterruptDisable()
		main --call onetime--> SignalOutput_ModuleInit()
		main --call in while loop--> Lde_Control
	*/
	void Servo_Init()
{
	// cau hinh gpio pin
