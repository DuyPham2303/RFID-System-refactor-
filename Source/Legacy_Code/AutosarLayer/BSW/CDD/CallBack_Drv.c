...
static void (*adc_pCb[MAX_INSTANCE_ADC])(void) = {NULL, NULL};
static void (*timer_pCb[MAX_INSTANCE_ADC])(void) = {NULL, NULL, NULL};

/* Publish API --> Service sử dụng */
Std_ReturnType Brake_RegisterCallBack(uint8_t instance_id, CallBack pCb)
{
    if (pCb != NULL)
    {
...
Std_ReturnType Dht11_RegisterCallBack(uint8_t instance_id, CallBack pCb)
{
    if (pCb != NULL)
    {
        if (instance_id <= MAX_INSTANCE_ADC)
...
void ADC1_2_IRQHandler()
{
    ...
    if (adc_pCb[BRAKE_ADC_ID_CB] != NULL)
        adc_pCb[BRAKE_ADC_ID_CB]();
    else if (adc_pCb[PEDAL_ADC_ID_CB] != NULL)
...
{
    if (timer_pCb[DHT11_TIMER_ID_CB] != NULL)
...
{
    if (timer_pCb[SERVO_TIMER_ID_CB] != NULL)
...
{
    if (timer_pCb[LED_TIMER_ID_CB] != NULL)
